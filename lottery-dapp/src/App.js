...
function App() {
  ...
  const connectWallet = async () => {
    ...
    const newContract = new Contract(CONTRACT_ADDRESS, DiceGameABI, signer);

    setProvider(newProvider);
    setContract(newContract);
    setAccount(accounts[0]);
    showMessage("✅ 錢包已連接");
    fetchBalance(accounts[0], newContract);

    // ⚡️ 監聽 DiceRolled 事件
    newContract.on("DiceRolled", (player, result, win, payout) => {
      if (player.toLowerCase() === accounts[0].toLowerCase()) {
        setTimeout(() => {
          clearInterval(rollInterval);
          setRolling(false);
          setDiceNumber(Number(result));
  ...
  const fetchBalance = async (address, _contract = contract) => {
    try {
      const bal = await _contract.balances(address);
